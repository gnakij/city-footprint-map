...
const CHANGELOG = [
  { date: '2026-06-20', items: ['地图城市标签弹窗边框颜色不再跟随城市的染色梯度变化（之前会因停留天数落在某些档位时显示蓝色等与主题不符的颜色），改为固定使用当前主题的品牌主色'] },
  { date: '2026-06-20', items: ['地图“点亮/未点亮”城市与省份标签颜色不再固定写死，已点亮统一改为当前主题的品牌主色（--color-primary），未点亮改为项目已有的次要文字色（--color-on-surface-variant），三个主题各自天然就有协调的专属值，不新造颜色；之前固定的深酒红/浅紫灰不会跟随主题切换'] },
  { date: '2026-06-20', items: ['修复管理员/个人面板（用户名昵称/访问记录/系统管理/系统设置四个页签）切换时整体高度忽上忽下的问题：根因是弹窗容器只设置了最大高度没有最小高度，不同页签内容量差异很大（“个人信息”仅两个输入框，“系统管理”嵌套完整表格界面），已加上最小高度，移动端额外做了适配避免在小屏幕上过高溢出', '调整管理表格表头字号：从12px调到13px（与页签同字号），并加粗为700字重，解决“列表内容字体比表头还大”的问题，同时表头与内容（14px）只差1px不会显得头重脚轻'] },
  { date: '2026-06-20', items: ['修复统计面板边框“隐隐发红”的问题：根因是.card统一阴影(--shadow-card/--shadow-glow)里硬编码了旧主色的RGB分量，上次调整主色时漏检了这两处，已同步更新为新主色RGB，登录页背景渐变同样处理', '地图悬停预览色不再固定为黄色#FFD166，改为复用此前未被使用的--color-warning变量，三主题各自配专属色值（Rose/Linear/Stripe），经验证不会与各主题自身的染色梯度撞色'] },
  { date: '2026-06-20', items: ['Rose主题主色微调：--color-primary 由 #F95D84 改为 #E0083E，修复主按钮（保存、确认等高频操作按钮）白色文字对比度不达标的问题（3.03:1→4.91:1，达到WCAG AA标准），同时连带改善了主色用于图标/品牌标识场景下的对比度（2.95:1→4.79:1）；用户从“加深主色本身”与“只换按钮文字颜色”两个方案、以及五档不同深浅程度的效果图里选定当前值，整体仍保持温馨粉色调，没有偏向纯红', '已同步更新设计系统文档里的色彩系统主表（此前该表色值与代码实际定义大范围不一致，怀疑是早期设计草稿色值，代码实现后从未同步更新过文档），现已用脚本逐项核对替换为真实色值'] },
