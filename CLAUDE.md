...
**待处理（已讨论但未实施）**：
- 地图"点亮城市"标签颜色（`MapView.tsx` 的 `LABEL_COLOR_LIT`/`LABEL_COLOR_UNLIT`，
  当前固定为深红`#8C2540`/浅紫`#B7A2AA`）同样不跟随主题，用户已确认希望
  跟随主题变化，但尚未选定三主题各自的具体色值，需要先出效果图给用户选。
- 地图标签弹窗（tooltip）边框颜色"看起来是蓝色"：根因是 ECharts 默认行为
  会取被点击数据项本身的颜色（`tooltipDataParams.color`）作为边框色，不是
  写死的蓝色——城市按停留天数染色后若落在`--color-dur-l2`/`l3`这类中间档
  （恰好是蓝色调，因为染色梯度本身用的是蓝色系），边框就会跟着显示蓝色。
  这不是"设置错了一个颜色"，是"要不要显式指定一个固定的tooltip边框色，
  替代这种取决于数据的默认行为"的设计判断，需要用户决定。

**讨论过但需要用户先做设计判断、本次未动**：
- 顶部Logo只有文字"城市足迹"，无图标、无slogan——内容缺失，不是bug，
  需要用户提供或确认是否要加图标/slogan及具体内容。
...
- `.data-table th`(表头,12px) 字号小于 `.data-table td`(内容,14px)，
  用户反馈"列表字体比表头都大"，已确认数值差异真实存在，但用户自己提到
  "表头字号调大可能和上面的tab等元素不协调"，需要综合考虑后再定具体方案。
